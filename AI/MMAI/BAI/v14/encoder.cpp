...
using Encoding = Schema::V14::Encoding;
...
#define MAYBE_ADD_MASKED_AND_RETURN(v, n, out)                 \
	...
	{                                                          \
		(out).insert((out).end(), n, S14::NULL_VALUE_ENCODED); \
		return;                                                \
	}
...
void Encoder::Encode(const S14::HexAttribute a, int v, BS & out)
{
	const auto & [_, e, n, vmax, p] = S14::HEX_ENCODING.at(EI(a));
	Encode(EncoderInput{.attrname = "HexAttribute", .a = EI(a), .e = e, .n = n, .vmax = vmax, .p = p, .v = v}, out);
}

void Encoder::Encode(const S14::PlayerAttribute a, int v, BS & out)
{
	const auto & [_, e, n, vmax, p] = S14::PLAYER_ENCODING.at(EI(a));
	Encode(EncoderInput{.attrname = "PlayerAttribute", .a = EI(a), .e = e, .n = n, .vmax = vmax, .p = p, .v = v}, out);
}

void Encoder::Encode(const S14::GlobalAttribute a, int v, BS & out)
{
	const auto & [_, e, n, vmax, p] = S14::GLOBAL_ENCODING.at(EI(a));
...
{
	out.push_back(v == S14::NULL_VALUE_UNENCODED);
...
void Encoder::EncodeExpnormMaskingNull(int v, int vmax, double slope, BS & out)
{
	if(v == S14::NULL_VALUE_UNENCODED)
	{
		out.push_back(S14::NULL_VALUE_ENCODED);
		return;
	}
	EncodeExpnorm(v, vmax, slope, out);
}
