...
{
	auto [stacks, queue] = InitStacks(cache, battle, acstack, oldgstats, gstats, stacksStats, isMorale);
	auto [hexes, astack] = InitHexes(cache, battle, acstack, stacks);
...
Queue Battlefield::GetQueue(const CPlayerBattleCallback * battle, const CStack * astack, bool isMorale)
{
	...
	for(const auto & units : tmp)
	{
		...
		{
			if(res.size() < S14::STACK_QUEUE_SIZE)
