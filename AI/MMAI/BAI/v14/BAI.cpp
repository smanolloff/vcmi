...
Schema::Action BAI::getNonRenderAction()
{
	...
	while(action == Schema::ACTION_RENDER_ANSI)
	{
		...
		if(state->supdata->ansiRender.empty())
		{
			...
			state->supdata->type = Schema::V14::ISupplementaryData::Type::ANSI_RENDER;
		}

		action = model->getAction(state.get());
	}
	state->supdata->ansiRender.clear();
	state->supdata->type = Schema::V14::ISupplementaryData::Type::REGULAR;
	return action;
}
...
#ifdef ENABLE_ML
	if (roundcounter > Schema::V14::MAX_ROUNDS) {
		logger.warn("Max rounds (%d) exceeded, retreating...", Schema::V14::MAX_ROUNDS);
