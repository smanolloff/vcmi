...
using F1 = Schema::V14::StackFlag1;
using F2 = Schema::V14::StackFlag2;
...
namespace
{
	int calculateSlot(const CStack * cstack)
	{
		...
		{
			return S14::STACK_SLOT_WARMACHINES;
		}
		else
		{
			// "special" slot, e.g. summoned, commander, etc.
			return S14::STACK_SLOT_SPECIAL;
		}
	}
	...
	char calculateAlias(int slot)
	{
		switch(slot)
		{
			case S14::STACK_SLOT_SPECIAL:
				return 'S';
				break;
			case S14::STACK_SLOT_WARMACHINES:
				return 'M';
				break;
			default:
...
std::pair<BitQueue, int> Stack::QBits(const CStack * cstack, const Queue & vec)
{
	BitQueue q;
	int pos = -1;
	if(vec.size() != S14::STACK_QUEUE_SIZE)
...
#ifdef ENABLE_ML
	if(cstack->creatureId().num > Schema::V14::CREATURE_ID_MAX)
