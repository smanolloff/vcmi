	...
	, nullstack(InitNullStack())
{
	...
	gstats = std::make_unique<GlobalStats>(battle->battleGetMySide(), 0, lv + rv, lh + rh);
...
void State::_onActionStarted(const BattleAction & ba)
{
	...
	switch(ba.actionType)
	{
		case EActionType::WAIT:
			startedAction = S14::ACTION_WAIT;
			break;
		case EActionType::SHOOT:
		{
			auto bh = ba.target.at(0).hexValue;
			auto id = Hex::CalcId(bh);
			startedAction = S14::N_NONHEX_ACTIONS + id * EI(HexAction::_count) + EI(HexAction::SHOOT);
		}
		break;
		case EActionType::DEFEND:
		{
			...
			auto id = Hex::CalcId(bh);
			startedAction = S14::N_NONHEX_ACTIONS + id * EI(HexAction::_count) + EI(HexAction::MOVE);
		}
		break;
		...
		case EActionType::WALK_AND_ATTACK:
		{
			...
			for(int i = 0; i < nbhexes.size(); ++i)
			{
				...
				if(n_bhex == bhTarget)
				{
					startedAction = S14::N_NONHEX_ACTIONS + idMove * EI(HexAction::_count) + i;
					break;
				}
			}
