...
using IHex = Schema::V15::IHex;
using IStack = Schema::V15::IStack;
using ISupplementaryData = Schema::V15::ISupplementaryData;
...
namespace
{
	...
	{
		auto vmax = std::get<3>(S15::HEX_ENCODING.at(EI(a)));
...
void Verify(const State * state) // NOSONAR - function used for debugging only
{
	...
	if(ended)
	{
		...
		if(fin.has_value())
		{
			...
			(fin == BattleSide::NONE) ? ensureValueMatch(gstats->getAttr(GA::BATTLE_WINNER), S15::NULL_VALUE_UNENCODED, "GA.BATTLE_WINNER (draw)")
	...
	else
	{
		...
		ensureValueMatch(gstats->getAttr(GA::BATTLE_WINNER), S15::NULL_VALUE_UNENCODED, "GA.BATTLE_WINNER (battle ongoing)");
			...
			switch(attr)
			{
				...
				case HA::STACK_SLOT:
				{
					...
					if(want == SlotID::WAR_MACHINES_SLOT)
						want = S15::STACK_SLOT_WARMACHINES;
					else if(want < 0 || want > 7)
						want = S15::STACK_SLOT_SPECIAL;
				...
				case HA::STACK_QUANTITY:
					ensureStackNullOrMatch(
						...
						[&cstack]
						{
							return std::round(S15::STACK_QTY_MAX * static_cast<float>(cstack->getCount()) / S15::STACK_QTY_MAX);
						},
						"HEX.STACK_QUANTITY"
					);
					break;
				...
				case HA::STACK_QUEUE:
				{
					...
					auto qbits = std::bitset<S15::STACK_QUEUE_SIZE>(v);

					for(int n = 0; n < S15::STACK_QUEUE_SIZE; ++n)
					{
...
std::string Render(const Schema::IState * istate, const Action * action) // NOSONAR - function used for debugging only
{
	...
	auto ended = gstats->getAttr(GA::BATTLE_WINNER) != S15::NULL_VALUE_UNENCODED;
	...
	for(int y = 0; y < 11; y++)
	{
		for(int x = 0; x < 15; x++)
		{
			...
			if(stack)
			{
				...
				constexpr int n = std::get<2>(S15::HEX_ENCODING[EI(HA::STACK_FLAGS1)]);
	...
	for(const auto & [a, aname] : rowdefs)
	{
		...
		for(auto side : {0, 1})
		{
			...
			for(int i = 0; i < sidestacks.size(); ++i)
			{
				...
				// MSVC mandates constexpr `n` here
				constexpr int n1 = std::get<2>(S15::HEX_ENCODING[EI(HA::STACK_FLAGS1)]);
				constexpr int n2 = std::get<2>(S15::HEX_ENCODING[EI(HA::STACK_FLAGS2)]);
				...
				if(a == SA::QUEUE)
				{
					auto qbits = std::bitset<S15::STACK_QUEUE_SIZE>(stack->getAttr(SA::QUEUE));
					for(int n = 0; n < S15::STACK_QUEUE_SIZE; ++n)
					{
						if(qbits.test(n))
						{
