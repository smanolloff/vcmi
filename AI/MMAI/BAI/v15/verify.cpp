...
namespace
{
		...
		for (const auto & unit : nodes)
		{
			for (int i = 0; i < EU(A::_count); ++i)
			{
				...
				case A::HAS_AGE_ATTACK:
					vassert(v, 10 * cstack.valOfBonuses(BonusType::SPELL_AFTER_ATTACK, SpellID(SpellID::AGE)), "UNIT.HAS_AGE_ATTACK", cstack.getDescription());
					break;
				case A::HAS_BIND:
				...
				case A::HAS_BIND_ATTACK:
					vassert(v, 10 * cstack.valOfBonuses(BonusType::SPELL_AFTER_ATTACK, SpellID(SpellID::BIND)), "UNIT.HAS_BIND_ATTACK", cstack.getDescription());
					break;
				case A::HAS_BLIND:
					{
				...
				case A::HAS_CURSE_ATTACK:
					vassert(v, 10 * cstack.valOfBonuses(BonusType::SPELL_AFTER_ATTACK, SpellID(SpellID::CURSE)), "UNIT.HAS_CURSE_ATTACK", cstack.getDescription());
					break;
				case A::HAS_DISPEL_ATTACK:
					vassert(v, 10 * cstack.valOfBonuses(BonusType::SPELL_AFTER_ATTACK, SpellID(SpellID::DISPEL_HELPFUL_SPELLS)), "UNIT.HAS_DISPEL_ATTACK", cstack.getDescription());
					break;
				case A::HAS_PETRIFY:
				...
				case A::HAS_PETRIFY_ATTACK:
					vassert(v, 10 * cstack.valOfBonuses(BonusType::SPELL_AFTER_ATTACK, SpellID(SpellID::STONE_GAZE)), "UNIT.HAS_PETRIFY_ATTACK", cstack.getDescription());
					break;
				case A::HAS_POISON:
				...
				case A::HAS_POISON_ATTACK:
					vassert(v, 10 * cstack.valOfBonuses(BonusType::SPELL_AFTER_ATTACK, SpellID(SpellID::POISON)), "UNIT.HAS_POISON_ATTACK", cstack.getDescription());
					break;
				case A::HAS_WEAKNESS:
				...
				case A::HAS_WEAKNESS_ATTACK:
					vassert(v, 10 * cstack.valOfBonuses(BonusType::SPELL_AFTER_ATTACK, SpellID(SpellID::WEAKNESS)), "UNIT.HAS_WEAKNESS_ATTACK", cstack.getDescription());
					break;
				default:
	...
	void Verify_EDGE_ACTION_BLOCKS_UNIT(const Context & ctx)
	{
		for(const auto & edge : ctx.G.getAll<E::Action_Blocks_Unit>())
		{
			...
			expect(blocker.unitSide() != blocked.unitSide() || blocked.hasBonusOfType(BonusType::ATTACKS_NEAREST_CREATURE), "ACTION_BLOCKS_UNIT: friendly unit blocks non-berserk shooter");
			...
			expect(ctx.G.getEdgeBySrcDst<E::Unit_BlockedBy_Action>(edge->dstNode, edge->srcNode, false) != nullptr, "ACTION_BLOCKS_UNIT: no corresponding reverse edge");
		}
	}
	...
	void Verify_EDGE_UNIT_BECOMES_MELEE_THREAT_AFTER_ACTION(const Context & ctx)
	{
		for (const auto & edge : ctx.G.getAll<E::Unit_BecomesMeleeThreatAfter_Action>())
		{
			...
			knownAccessible.eraseIf([&action](const auto & bhex)
			{
			...
			bool isThreat = std::ranges::any_of(attackableHexes, [&speed, &reachability](const BattleHex & bh) {
	...
	void Verify_EDGE_UNIT_BECOMES_SHOOT_TARGET_AFTER_ACTION(const Context & ctx)
	{
		for (const auto & edge : ctx.G.getAll<E::Unit_BecomesShootTargetAfter_Action>())
		{
			...
			{
				const auto & blockers = ctx.battle.battleGetUnitsIf([&nbhex, &actor](const battle::Unit * u)
				{
					// NOTE: this will be incorrect if either stack is berserk
	...
	for (int i = 0; i < EU(S15::Graph::ElementType::_count); ++i)
	{
		...
		case ET::EDGE_PLAYER_OWNS_UNIT:
			for (const auto & edge : ctx.G.getAll<E::Player_Owns_Unit>())
			{
				...
				expect(ctx.G.getEdgeBySrcDst<E::Unit_OwnedBy_Player>(edge->dstNode, edge->srcNode) != nullptr, "EDGE_PLAYER_OWNS_UNIT: no corresponding reverse edge");
			}
		...
		case ET::EDGE_UNIT_OWNED_BY_PLAYER:
			for (const auto & edge : ctx.G.getAll<E::Unit_OwnedBy_Player>())
			{
				...
				expect(ctx.G.getEdgeBySrcDst<E::Player_Owns_Unit>(edge->dstNode, edge->srcNode) != nullptr, "EDGE_UNIT_OWNED_BY_PLAYER: no corresponding reverse edge");
			}
			break;
		...
		case ET::EDGE_ACTION_BY_UNIT:
			...
			for (const auto & edge : ctx.G.getAll<E::Action_By_Unit>())
			{
				...
				expect(ctx.G.getEdgeBySrcDst<E::Unit_Has_Action>(edge->dstNode, edge->srcNode) != nullptr, "EDGE_ACTION_BY_UNIT: no corresponding reverse edge");
			}
			break;
