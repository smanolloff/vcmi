...
using HexStateMask = std::bitset<EI(HS::_count)>;
...
class Hex : public S15::Graph::INode
{
public:
	...
	{
		return S15::Graph::NodeType::HEX;
	}

	std::vector<float> encodedAttributes() const override
	{
		return encodeNodeAttributes(attrs, S15::HEX_ENCODING);
	}

	int getAttr(HA a) const
	{
		return attr(a);
	}
...
private:
	...
	void finalize()
	{
		attrs.at(EI(HA::ACTION_MASK)) = static_cast<int>(actmask.to_ulong());
		attrs.at(EI(HA::STATE_MASK)) = static_cast<int>(statemask.to_ulong());
	}
	...
	)
	{
		for(const auto & obstacle : obstacles)
		{
			switch(obstacle->obstacleType)
			{
				...
				case CObstacleInstance::ABSOLUTE_OBSTACLE:
					statemask.reset(EI(HS::PASSABLE));
					break;
				case CObstacleInstance::MOAT:
					if(!(bhex == BattleHex::GATE_BRIDGE && isGateOpen))
					{
						statemask.set(EI(HS::STOPPING));
						statemask.set(EI(HS::DAMAGING_L));
						statemask.set(EI(HS::DAMAGING_R));
					}
					break;
				case CObstacleInstance::SPELL_CREATED:
					switch(SpellID(obstacle->ID))
					{
						...
						case SpellID::LAND_MINE:
						{
							...
							if(side == casterSide)
								statemask.set(EI(side == BattleSide::DEFENDER ? HS::DAMAGING_L : HS::DAMAGING_R));
							else
								statemask.set(EI(side == BattleSide::DEFENDER ? HS::DAMAGING_R : HS::DAMAGING_L));
							break;
						}
						default:
							break;
		...
		if(bhex == BattleHex::GATE_INNER || bhex == BattleHex::GATE_OUTER)
			statemask.set(EI(HS::SIEGE_GATE));
		else if(bhex == BattleHex::GATE_BRIDGE)
			statemask.set(EI(HS::SIEGE_BRIDGE));
			...
			statemask.set(EI(HS::SIEGE_WALL));
	}
