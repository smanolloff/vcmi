...
{
public:
    static std::shared_ptr<const Unit_BecomesShootThreatAfter_Action> Create(
    ...
    Unit_BecomesShootThreatAfter_Action(
        const std::shared_ptr<const Nodes::Unit> & srcNode,
        const std::shared_ptr<const Nodes::Action> & dstNode,
        float mult)
    : detail::Unit_BecomesShootThreatAfter_Action_Base(srcNode, dstNode)
    , mult(mult)
    {
