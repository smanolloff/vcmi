...
{
public:
    static std::shared_ptr<const Hex_BecomesShootTargetAfter_Action> Create(
    ...
    Hex_BecomesShootTargetAfter_Action(
        const std::shared_ptr<const Nodes::Hex> & srcNode,
        const std::shared_ptr<const Nodes::Action> & dstNode,
        float mult)
    : detail::Hex_BecomesShootTargetAfter_Action_Base(srcNode, dstNode)
    , mult(mult)
    {
