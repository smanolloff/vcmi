...
namespace
{
	...
	void FixMoatMasks(const CPlayerBattleCallback * battle, const Hexes * hexes, const Stack * astack)
	{
		...
		auto noAmove = HexActionMask(0b11000000000000); // leftmost bit = bit#0
		...
		for(int row = 0; row < 11; ++row)
		{
			for(int col = 0; col < 15; ++col)
			{
				...
				// XXX: In vanilla H3, flying can always attack from a STOPPING
				// hex, even when not already standing on it.
				// However, in VCMI this logic is different (as of Mar 2026).
				// When that changes, add an if+continue check for flyers here.
				...
				// Moving onto any hex such that our occupied hex is on
				// a stopping hex aborts the attack => disallow
				if(isAttacker && col > 0)
				{
