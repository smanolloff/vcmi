...
namespace MMAI::BAI::V3 {
    ...
    bool Hex::isOccupied() const { return getState() == HexState::STACK_FRONT || getState() == HexState::STACK_BACK; }
    ...
    void Hex::setShootDistanceFromStack(bool isActive, bool isRight, int slot, ShootDistance distance) {
        if (isActive)
            setShootDistanceFromAStack(distance);
    ...
    void Hex::setCStackAndAttrs(const CStack* cstack_, int qpos) {
        ...
        // STACK_SLOT=7 is for summoned creatures (-3), war machines (-4), arrow towers (-5)
        auto slot = cstack->unitSlot() < 0 ? STACK_SPECIAL_SLOT : cstack->unitSlot();
        setattr(A::STACK_SLOT,                  slot);
