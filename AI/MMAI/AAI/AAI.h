...
public:
	...
	void heroGotLevel(
		...
		QueryID queryID
	) override; //pskill is gained primary skill, interface has to choose one of given skills and call callback with selection id
	...
	) override; //Show a dialog, player must take decision. If selection then he has to choose between one of given components, if cancel he is allowed to not choose. After making choice, CCallback::selectionMade should be called with number of selected component (1 - n) or 0 for cancel (if allowed) and askID.
	void showGarrisonDialog(
	...
	void showMapObjectSelectDialog(
		QueryID askID,
...
private:
	...
	template<typename... Args>
	void log(const ELogLevel::ELogLevel level, const std::string & format, Args... args) const;
	template<typename... Args>
