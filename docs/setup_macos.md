...
```bash
$ ./vcmibuilder --data /path/to/heroes3 --dest data
$ ln -s $(readlink -f ../maps/gym) data/Maps/gym
```

Note: The heroes3 folder must contain the following directories:
...
```bash
rel/bin/mlclient-cli --map gym/A1.vmap --blue-ai MMAI_MODEL --blue-model /path/to/model.onnx
```

where `/path/to/model.onnx` is a pre-trained Torch ONNX model. If you don't have
